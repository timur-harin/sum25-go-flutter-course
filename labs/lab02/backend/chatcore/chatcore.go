...
func (b *Broker) Run() {
	// Implement event loop (fan-in/fan-out pattern)
	for {
		select {
		case <-b.ctx.Done():
			close(b.done)
...
func (b *Broker) SendMessage(msg Message) error {
	// Send message to appropriate channel/queue
	select {
	case b.input <- msg:
		return nil
	case <-b.ctx.Done():
...
func (b *Broker) RegisterUser(userID string, recv chan Message) {
	// Register user and their receiving channel
	b.usersMutex.Lock()
	b.users[userID] = recv
	b.usersMutex.Unlock()
}

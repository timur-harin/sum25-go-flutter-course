...
class AuthService {
  ...
  // login authenticates a user with email and password. It validates the
  // credentials, fetches the user from the repository, verifies the password,
  // generates a token and updates the authentication state. Returns an
  // AuthResult describing the outcome.
  ...
  // isSessionValid checks if the current session is still valid. Sessions
  ...
  // refreshAuth validates and refreshes the current authentication status. It
  // ensures the session and stored token are still valid.
  ...
  // getUserInfo returns user information if authenticated. The returned map
  // includes the user's id, name, email, login time and whether the session is
  // currently valid.

...
class AuthService {
  ...
  Future<AuthResult> login(String email, String password) async {
    ...
    final verify = await _userRepository.verifyPassword(email, password);
    if (verify == false) {
      return AuthResult.invalidCredentials;
    }
  ...
  bool isSessionValid() {
    ...
    return DateTime.now().difference(_currentState.loginTime!) < Duration(hours: 24);
  }
  ...
  Future<bool> refreshAuth() async {
    ...
    if (_currentState.token != null && _jwtService.validateToken(_currentState.token!)) {
      return true;
    } else {
      logout();
      return false;

...
class AuthService {
  ...
  Future<AuthResult> login(String email, String password) async {
    try {
      ...
      final passOk = await _userRepository.verifyPassword(email, password);
      if(!passOk) {
        return AuthResult.invalidCredentials;
      }

      final token = _jwtService.generateToken(user.id.toString(), user.email);
      _currentState = AuthState(isAuthenticated: true, currentUser: user, token: token, loginTime: DateTime.now());
  ...
  bool isSessionValid() {
    ...
    final timeDifference = DateTime.now().difference(_currentState.loginTime!);
  ...
  Map<String, dynamic>? getUserInfo() {
    if(_currentState.currentUser == null || !_currentState.isAuthenticated) {
      return null;
    }
    else {
      return {

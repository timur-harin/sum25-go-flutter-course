...
class AuthService {
  ...
  Future<AuthResult> login(String email, String password) async {
    ...
    try {
      ...
      final isPasswordValid =
          await _userRepository.verifyPassword(user.email, password);
      if (!isPasswordValid) {
        return AuthResult.invalidCredentials;
      }
      ...
      // Generate JWT token
      final token =
          await _jwtService.generateToken(user.id.toString(), user.email);

      // Update current state with authenticated user info
      ...
      // Handle any exception as a network error
      return AuthResult.networkError;
    }
  }

  Future<void> logout() async {
    // Reset the authentication state to clear all user info
    _currentState = AuthState();
  }
  ...
  bool isSessionValid() {
    ...
    return duration < Duration(hours: 24);
  }
  ...
  Future<bool> refreshAuth() async {
    try {
      ...
      final claims = await _jwtService.validateToken(token);
      if (claims == false) {
        await logout();
        return false;
      }

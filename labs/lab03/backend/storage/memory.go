...
func (ms *MemoryStorage) GetAll() ([]*models.Message, error) {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *MemoryStorage) Update(id int, content string) (*models.Message, error) {
	...
	// Create a new message with updated content but same ID and timestamp
	updated := models.NewMessage(id, message.Username, content)
	updated.Timestamp = message.Timestamp // Preserve original timestamp
	ms.messages[id] = updated
	return updated, nil
}

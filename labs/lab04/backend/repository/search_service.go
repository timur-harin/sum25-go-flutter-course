...
func (s *SearchService) SearchPosts(ctx context.Context, filters SearchFilters) ([]models.Post, error) {
	...
	// WHERE: MinWordCount (count words in content)
	if filters.MinWordCount != nil {
		// PostgreSQL: array_length(regexp_split_to_array(content, '\s+'), 1) >= ?
		query = query.Where(
			squirrel.Expr("array_length(regexp_split_to_array(content, '\\s+'), 1) >= ?", *filters.MinWordCount),
		)
	}
	...
	orderDir := "DESC"
	if filters.OrderDir == "ASC" || filters.OrderDir == "asc" {
		orderDir = "ASC"
	}
...
func (s *SearchService) GetTopUsers(ctx context.Context, limit int) ([]UserWithStats, error) {
	query := s.psql.
		Select(
			...
			"COALESCE(MAX(p.created_at)::text, '') AS last_post_date",
		).
		From("users u").
